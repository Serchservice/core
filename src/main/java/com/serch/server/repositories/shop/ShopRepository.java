...
public interface ShopRepository extends JpaRepository<Shop, String> {
    ...
    @Query(
            value = "SELECT shop.* FROM platform.shops shop " +
                    ...
                    "LEFT JOIN verified.verification verify ON use.id = verify.serch_id " +
                    "WHERE " +
                    "to_tsvector('english', serv.service) @@ plainto_tsquery(:query) " +
                    ...
                    "shop.updated_at, address, shop.category, shop.name, shop.phone_number " +
                    "ORDER BY " +
                    "CASE verify.status" +
