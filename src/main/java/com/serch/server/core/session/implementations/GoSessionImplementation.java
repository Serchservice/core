...
import com.serch.server.setups.adapters.LocalDateGsonAdapter;
...
public class GoSessionImplementation implements GoSessionService {
    ...
    @Override
    public GoAuthResponse response(GoUser user) {
        GoAuthResponse response = GoMapper.instance.auth(user);
        response.setSession(encode(user.getId(), user.getEmailAddress()));

        return response;
    }

    @SneakyThrows
    String encode(UUID id, String emailAddress) {
        String json = gson.toJson(new GoSession(
                id,
                emailAddress,
    ...
    @SneakyThrows
    @Override
    public GoSession decode(String token) {
        ...
        return gson.fromJson(json, new TypeToken<GoSession>() {}.getType());
    }

    @Override
    public ApiResponse<String> validateSession(String token) {
        GoSession session = decode(token);
        ...
        if(keyValidator.isSigned(new String(Decoders.BASE64.decode(session.getKey())))) {
            GoUser user = goUserRepository.findById(session.getId())
                    .orElseThrow(() -> new AuthException("Invalid session details"));

            return new ApiResponse<>("Successful", user.getEmailAddress(), HttpStatus.OK);
        } else {
