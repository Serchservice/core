package com.serch.server.admin.services.auth.services.implementations;

import com.serch.server.admin.enums.ActivityMode;
import com.serch.server.admin.enums.UserAction;
import com.serch.server.admin.mappers.AdminAuthMapper;
...
public class AdminAuthImplementation implements AdminAuthService {
    ...
    public ApiResponse<AuthResponse> finishSignup(FinishAdminSetupRequest request) {
        User user = userRepository.findById(UUID.fromString(jwtService.getItemFromToken(request.getSecret(), "user")))
        ...
        if(isSecretValid(request.getSecret())) {
            ...
            if(HelperUtil.validatePassword(request.getPassword())) {
                ...
                createAdminActivityAfterFinishingSignup(user);

                return authService.getAuthResponse(AdminAuthMapper.instance.login(request), user);
            } else {
                throw new AuthException("Password must contain a lowercase, uppercase, special character and number");
            }
        } else {
    ...
    private void createAdminActivityAfterFinishingSignup(User user) {
    ...
    private boolean isSecretValid(String secret) {
        ...
        return jwtService.isTokenIssuedBySerch(secret)
                && adminRepository.existsById(UUID.fromString(jwtService.getItemFromToken(secret, "admin")))
                && userRepository.existsById(userId)
                && user.getRole() == Role.valueOf(jwtService.getItemFromToken(secret, "role"))
                && user.getAction() == UserAction.valueOf(jwtService.getItemFromToken(secret, "action"))
    ...
    public ApiResponse<MFADataResponse> verifyLink(String secret) {
        if(isSecretValid(secret)) {
            User user = userRepository.findById(UUID.fromString(jwtService.getItemFromToken(secret, "user")))
                    .orElseThrow(() -> new AuthException("User not found"));

            return new ApiResponse<>(user.getFirstName(), mfaService.getMFAData(user), HttpStatus.OK);
        } else {
    ...
    public ApiResponse<String> verifyResetLink(String token) {
        if(isSecretValid(token)) {
            User user = userRepository.findById(UUID.fromString(jwtService.getItemFromToken(token, "user")))
                    .orElseThrow(() -> new AuthException("User not found"));
    ...
    public ApiResponse<AuthResponse> resetPassword(AdminResetPasswordRequest request) {
        User user = userRepository.findById(UUID.fromString(jwtService.getItemFromToken(request.getToken(), "user")))
                .orElseThrow(() -> new AuthException("User not found"));
