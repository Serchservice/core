...
public interface TripRepository extends JpaRepository<Trip, String> {
    ...
    @Query("select t from Trip t where t.account = ?1 and t.status = 'ACTIVE' and (t.invited is null or (t.invited is not null and t.access = 'GRANTED' and t.invited.status = 'ACTIVE'))")
    Page<Trip> findByActiveUserTrips(@NonNull String account, Pageable pageable);

    @Query("select t from Trip t where t.account = ?1 and t.linkId = ?2 and t.status = 'ACTIVE' and (t.invited is null or (t.invited is not null and t.access = 'GRANTED' and t.invited.status = 'ACTIVE'))")
    ...
    @Query("""
        ...
        and t.status = 'ACTIVE'
        and (t.invited is null or (t.invited is not null and t.access = 'GRANTED' and t.invited.status = 'ACTIVE'))
    """)
    Page<Trip> findByActiveProviderTrips(@NonNull UUID id, Pageable pageable);

    @Query("""
        select t from Trip t where t.account = :account
        and (t.status = 'UNFULFILLED' or t.status = 'CLOSED')
        and (:category is null or (:category is not null and t.category = :category))
        and (TRUE = :#{#dateTime == null} or t.createdAt = :dateTime)
        and (:isShared is null or (t.invited is not null and :isShared = true))
    """)
    ...
    @Query("""
        select t from Trip t where t.account = :account and t.linkId = :linkId
        and (t.status = 'UNFULFILLED' or t.status = 'CLOSED')
        and (:category is null or (:category is not null and t.category = :category))
        and (TRUE = :#{#dateTime == null} or t.createdAt = :dateTime)
        and (:isShared is null or (t.invited is not null and :isShared = true))
    ...
    @Query("""
        ...
        left join t.invited.provider inp
        left join t.invited.provider.business inpb
    ...
    @Query("select t from Trip t where t.account = ?1 and t.status = 'ACTIVE' and t.invited is not null and t.access = 'GRANTED' and t.invited.status = 'WAITING'")
    Page<Trip> findBySharedUserTrips(@NonNull String account, Pageable pageable);

    @Query("select t from Trip t where t.account = ?1 and t.linkId = ?2 and t.status = 'ACTIVE' and t.invited is not null and t.access = 'GRANTED' and t.invited.status = 'WAITING'")
    Page<Trip> findBySharedGuestTrips(@NonNull String account, @NonNull String linkId, Pageable pageable);

    @Query("select t from Trip t left join t.invited ti left join t.provider p left join p.business b left join ti.provider tip left join tip.business tib " +
            "where ((p.id = ?1 or b.id = ?1) or (tip.id = ?1 or tib.id = ?1)) and t.status = 'ACTIVE' and t.invited is not null and t.access = 'GRANTED' and t.invited.status = 'WAITING'"
    )
