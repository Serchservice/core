...
public interface ServiceKeywordRepository extends JpaRepository<ServiceKeyword, Long> {
    List<ServiceKeyword> findByCategory(@NonNull SerchCategory category);
    @Query(
            value = "SELECT * FROM " +
                    "(SELECT *, " +
    ...
    List<ServiceKeyword> fullTextSearch(@Param("query") String query);
