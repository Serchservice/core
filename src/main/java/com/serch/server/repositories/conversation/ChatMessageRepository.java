...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, String> {
    ...
    @Query("SELECT COUNT(m) FROM chat_messages m WHERE m.chatRoom.id = :roomId AND m.sender != :senderId  AND m.status != 'READ' and m.state = 'ACTIVE'")
    Long countMessagesReceivedByUser(@Param("roomId") String roomId, @Param("senderId") UUID senderId);
