...
import com.serch.server.core.email.template.EmailAuthTemplate;
...
public class EmailTemplateSender implements EmailTemplateService {
    ...
    @Override
    public ApiResponse<SendEmailResponse> sendLogin(String to, String content, String name) {
        Email email = new Email();
        email.setEmailAddress(to);
        email.setOtp(content);
        email.setCentered(false);
        email.setContent("Confirm your identity with the One-time password sent for your login request");
        email.setGreeting("Hey %s".formatted(name));
        email.setImageHeader("/messages.png?alt=media&token=2c2e888d-67ad-46e4-b74e-ff62f21c2683");

        return service.send(
                "team@serchservice.com",
                to,
                "Login to your account - Verify your email address",
                EmailAuthTemplate.email(email),
                true
        );
    }
    ...
    public ApiResponse<SendEmailResponse> send(SendEmail email) {
        return switch (email.getType()) {
            ...
            case LOGIN -> sendLogin(email.getTo(), email.getContent(), email.getFirstName());
