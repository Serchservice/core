...
public class RatingImplementation implements RatingService {
    ...
    public ApiResponse<String> rate(RateRequest request) {
        callRepository.findById(request.getId()).ifPresentOrElse(call ->
                rateCall(call, request), () -> tripRepository.findById(request.getId())
    ...
    public List<RatingChartResponse> buildChart(String id) {
        ...
        // Populate the location with default values for current month, last month, and the month before that
        ...
        // Update the location with actual response
