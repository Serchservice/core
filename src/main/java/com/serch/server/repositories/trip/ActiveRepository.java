...
public interface ActiveRepository extends JpaRepository<Active, Long> {
    ...
    @Query(
            ...
            "GROUP BY ap.id, plan.plan.type, plan.child.type, p.verification.status, ap.latitude, ap.longitude, " +
            "p.rating, ap.tripStatus " +
            "ORDER BY " +
            "SQRT(POWER(:latitude - ap.latitude, 2) + POWER(:longitude - ap.longitude, 2)) * 111.325 ASC, " +
            "CASE " +
            "WHEN plan.plan.type = 'PREMIUM' THEN 0 " +
            "WHEN plan.plan.type = 'ALL_DAY' THEN 1 " +
            "ELSE 2 " +
            "END ASC, " +
            "CASE " +
            "WHEN plan.plan.type = 'PREMIUM' THEN " +
            "CASE CAST(plan.child.type AS string) " +
    ...
    @Query(
            value = "SELECT ap FROM Active ap " +
                    ...
                    "GROUP BY ap.id, plan.plan.type, plan.child.type, p.verification.status, ap.latitude, ap.longitude, ap.tripStatus " +
                    "ORDER BY " +
                    "SQRT(POWER(:latitude - ap.latitude, 2) + POWER(:longitude - ap.longitude, 2)) * 111.325 ASC, " +
                    "CASE " +
                    "WHEN plan.plan.type = 'PREMIUM' THEN 0 " +
    ...
    @Query(
            value = "SELECT ap.* FROM platform.active_providers ap " +
                    ...
                    "GROUP BY ap.id, parent.type, child.type, sk.keyword, sk.category," +
                    " verify.status, ap.latitude, ap.longitude, ap.status " +
                    "ORDER BY " +
    ...
    @Query(
            value = "SELECT ap.* FROM platform.active_providers ap " +
                    ...
                    "GROUP BY ap.id, parent.type, child.type, sk.keyword, sk.category," +
                    " verify.status, ap.latitude, ap.longitude " +
                    "ORDER BY " +
                    "SQRT(POWER(:latitude - ap.latitude, 2) + POWER(:longitude - ap.longitude, 2)) * 111.325, " +
                    "CASE WHEN parent.type = 'PREMIUM' THEN 0 " +
                    "WHEN parent.type = 'ALL_DAY' THEN 1 " +
